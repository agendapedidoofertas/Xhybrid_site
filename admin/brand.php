...
?>
      <header class="page-header" style="padding-top:0;text-align:left;margin:0;max-width:none;">
        ...
        <p>Identidade do site (nome, SEO, cidade, área e urgência). Só administradores alteram.</p>
      </header>
      ...
      <form method="post" class="contact-form admin-form admin-form--wide" style="margin-top:1.5rem;">
        ...
        <?php foreach ($defs as $key => $def): ?>
          <?php if (($def['group'] ?? '') !== 'brand') continue; ?>
          ...
          <?php endif; ?>
        <?php endforeach; ?>
        <p class="text-muted" style="margin-top:1rem;font-size:0.875rem;">Logo e favicon: use os slugs <code>logo</code> e <code>favicon</code> na aba Imagens.</p>
